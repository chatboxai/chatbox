...
- FABLE_REVIEW.md: **all P0/P1/P2-security items done, plus the Phase-4 quick UX wins (§9.1 a11y labels, §9.3 composer discoverability, §9.9 jotai-family migration, 2026-07-04)** (see FABLE_REVIEW §0 for the ✅ list). Remaining tiers: P2 (§6.3 tool-error unification) and the low/opportunistic list below. A11y note for new UI: icon-only buttons must get `aria-label` (pattern: `SessionList.tsx`); don't wrap hover-triggered `Menu.Target` buttons in `Tooltip` — the dropdown races it.
