...
- Packaged desktop builds ship hidden sourcemaps: the `!dist/**/*.map` exclusion in `electron-builder.yml` is commented out while electron.vite.config.ts emits `sourcemap: 'hidden'` for production. Uncommenting shrinks the asar but is a packaging change — verify with `qa:release:mac` + an asar map-file audit (task chip spawned 2026-07-05).
