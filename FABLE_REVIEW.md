...
1. **P2:** SEC-8 (prod CSP) · SEC-5 (node-fetch) · §6.3 tool-error unification · §9.1 a11y labels.
...
| **P2 — next cycle** | ✅ SEC-3 (main-process provider proxy → `webSecurity: true`, shipped 2026-07-03 — see §0) · SEC-8 (prod CSP) · SEC-5 (node-fetch) · tool-error unification (§6.3) · a11y labels (§9.1) | SEC-3 done; remaining P2 items are each ≤1 session. |
