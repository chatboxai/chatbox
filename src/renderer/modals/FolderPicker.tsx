...
import { moveSessionToFolder, useFolders } from '@/stores/sessionFolders'

const FolderPicker = NiceModal.create((props: { sessionId: string }) => {
  const modal = useModal()
  const { t } = useTranslation()
  const { folders } = useFolders()
  const currentFolderId = rendererApplication.sessionQueryBridge
    .getCachedSessionsMeta()
    .find((s) => s.id === props.sessionId)?.folderId
