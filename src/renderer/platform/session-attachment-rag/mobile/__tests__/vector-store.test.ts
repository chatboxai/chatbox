...
describe('SQLiteBlobVectorStore', () => {
  ...
  it('upsert saves vectors via executeSet in batch', async () => {
    ...
    expect(statements[0].statement).toContain('INSERT OR REPLACE INTO session_attachment_vectors')
    expect(statements[0].values[0]).toBe(1)
    expect(statements[0].values[1]).toBe(10)
  })

  it('query computes cosine similarity and bounds results to topK', async () => {
  ...
  it('deleteIndex deletes vectors for the given attachment', async () => {
    ...
    expect(mockDb.run).toHaveBeenCalledWith(
      expect.stringContaining('DELETE FROM session_attachment_vectors WHERE attachment_id = ?'),
      [42]
    )
  })
})
