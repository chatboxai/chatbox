...
function traceScenario<T>(testCase: string, execute: () => Promise<T> | T) {
  return runChatBridgeScenarioTrace(
    {
      slug: 'chatbridge-single-app-discovery',
...
describe('ChatBridge single-app tool discovery and invocation', () => {
  it('routes an explicit Chess request into the approved host-managed tool path', () =>
    traceScenario('routes an explicit Chess request into the approved host-managed tool path', async () => {
  ...
  it('keeps unrelated prompts chat-only and refuses ambiguous board-game prompts', () =>
    traceScenario('keeps unrelated prompts chat-only and refuses ambiguous board-game prompts', () => {
