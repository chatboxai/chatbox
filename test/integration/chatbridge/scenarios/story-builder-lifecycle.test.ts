...
describe('ChatBridge Story Builder lifecycle', () => {
  ...
  it('launches Story Builder, reads the latest draft, saves a checkpoint, and hands completion back to chat', async () => {
  ...
  it('fails closed when the credential handle expires before Story Builder can save and leaves a resumable host-owned state', async () => {
    let currentNow = 500
    const broker = createChatBridgeAuthBroker({
      now: () => currentNow,
      createId: () => 'story-builder-handle-1',
